...
export class BookTicketComponent implements OnInit {
  ...
  constructor(
    ...
    this.bookForm1 = this.fb.group({
      ...
      customerInfo: this.fb.group({
        email: [
          this.USERRECORDS.email,
          [Validators.email, Validators.required],
        ],
        phone: [
          this.USERRECORDS.phone,
          [Validators.required, Validators.pattern('^[0-9]{10}$')],
        ],
        name: [
          this.USERRECORDS.name,
  ...
  setCommission(event: any) {
    ...
    this.payableAmount =
      parseFloat(this.bookingdata.PriceArray.totalFare) + parseFloat(value);
  }
  ...
  submitForm2() {
    ...
    if (this.bookForm2.invalid) {
      ...
      this.agentPaymentService.paymentRequest(paymentParam).subscribe((res) => {
        if (res.status == 1) {
          if (res.data == 'SEAT UN-AVAIL') {
            ...
            this.agentPaymentStatusService
              ...
              .subscribe((res) => {
                if (res.status == 1) {
                  ...
                  this.agentPaymentStatusService
                    .GetPnr(this.bookTicketResponse.transaction_id)
                    .subscribe((res) => {
                      if (res.status == 1) {
                        this.pnr = res.data.pnr;
                        this.qrcode =
                          Constants.CONSUMER_BASE_URL + 'pnr/' + res.data.pnr;

                        window.history.replaceState(
                          {},
                          '',
                          '/#/agent/bookTicket/' +
                            this.bookTicketResponse.transaction_id,
                        );
                      }
                    });
